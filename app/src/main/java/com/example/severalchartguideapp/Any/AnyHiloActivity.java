...
//https://github.com/AnyChart/AnyChart-Android/blob/master/sample/src/main/java/com/anychart/sample/charts/HiloChartActivity.java
public class AnyHiloActivity extends AnyChartActivity{

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_anyhilo);

        // 날짜 형식 : 유닉스 타임스탬프(13자리)

        AnyChartView anyChartView = findViewById(R.id.any_hilo_chart);

        Stock stock = AnyChart.stock(); // stock 차트 형식

        Plot plot = stock.plot(0); // plot

        plot.yGrid(true) // Y축 그리드 설정
                .yMinorGrid(true); // Y축 보조 그리드 설정

        Table table = Table.instantiate("x"); // ?
        table.addData(getData()); // 데이터 넣기
        TableMapping mapping = table.mapAs("{'high': 'high', 'low': 'low'}"); // 데이터 넣은거에서 이름 다시 mapping

        Hilo hilo = plot.hilo(mapping); // mapping
        hilo.name("Temperature"); // 범례 이름

        hilo.tooltip().format("Max: {%High}&deg;<br/>Min: {%Low}&deg;"); // tooltip 포맷
        stock.tooltip().useHtml(true); // html 속성
