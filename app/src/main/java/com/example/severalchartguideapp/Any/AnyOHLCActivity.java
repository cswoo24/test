...
//https://github.com/AnyChart/AnyChart-Android/blob/master/sample/src/main/java/com/anychart/sample/charts/OHLCChartActivity.java
public class AnyOHLCActivity extends AnyChartActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_anyohlc);

        AnyChartView anyChartView = findViewById(R.id.any_ohlc_chart);

        Table table = Table.instantiate("x"); // X 기준
        table.addData(getData()); // 데이터 삽입

        TableMapping mapping = table.mapAs("{open: 'open', high: 'high', low: 'low', close: 'close'}"); // 테이블에 새롭게 mapping(dataentry에서 index를 실제로 가져옴)

        Stock stock = AnyChart.stock(); // stock chart 생성

        Plot plot = stock.plot(0); // index0(open) 데이터를 plot으로
        plot.yGrid(true)
                .xGrid(true)
                .yMinorGrid(true)
                .xMinorGrid(true);

        plot.ema(table.mapAs("{value: 'close'}"), 20d, StockSeriesType.AREA); // value값 close를 EMA(Exponential Moving Average. 지수이동평균)로 mapping
        ...
        stock.scroller().ohlc(mapping); // 스크롤러(하단에 날짜 조정하는 곳)에 ohlc 시리즈 mapping(table.mapAs 한 거)
        anyChartView.setChart(stock);
    }

    private List<DataEntry> getData() { // 날짜 : 유니티 타임스탬프
