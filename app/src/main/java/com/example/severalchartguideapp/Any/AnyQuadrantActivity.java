...
//https://github.com/AnyChart/AnyChart-Android/blob/master/sample/src/main/java/com/anychart/sample/charts/QuadrantChartActivity.java
public class AnyQuadrantActivity extends AnyChartActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_anyquadrant);

        AnyChartView anyChartView = findViewById(R.id.any_quadrant_chart);

        Scatter quadrant = AnyChart.quadrant();

        quadrant.padding(35d, 35d, 35d, 35d);

        Marker marker = quadrant.marker(data()); // marker chart library에 data insert
        marker.labels()
                .enabled(true)
                .fontFamily("Arial")
                .fontColor("#546e7a");
        marker.labels()
                ...
                .offsetY(0d)
                .format("{%Name}");
        marker.tooltip(false); // tooltip 켜지 않기

        quadrant.quarters( // 차트 내용(4분할 된 칸 text, title 등 setting)
                "{\n" +
                        "      rightTop: {\n" +
                        "        title: {\n" +
                        "          text: 'LEADERS',\n" +
        ...
        label.offsetX(-20d);
        label.text("Power to Perform &#8594;"); // 글자 뒤 문자 : 유니코드 화살표
        label.rotation(-90d); // 라벨 회전
    ...
    private List<DataEntry> data() { // data 중 anchor, offset들어가 있는 데이터들이 존재함
