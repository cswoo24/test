...
//https://github.com/AnyChart/AnyChart-Android/blob/master/sample/src/main/java/com/anychart/sample/charts/BubbleChartActivity.java
public class AnyBubbleActivity extends AnyChartActivity {

    AnyChartView anyBubbleChart;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_anybubble);

        anyBubbleChart = findViewById(R.id.any_bubble_chart);

        // 데이터 리스트(data)를 여러 개 만들지 않고 하나 만든 후 리스트를 비워서 다시 데이터 넣음. 라이브러리 scatter.java 에 정의되어 있음
        ...
        bubble.title().useHtml(true); // Html 속성 사용
        bubble.title()
        ...
        bubble.yGrid(true)
                .xGrid(true)
                .xMinorGrid(true) // x축 그리드 표시(사이값들)
                .yMinorGrid(true); // y축 그리드 표시(사이값들)

        bubble.minBubbleSize(5d) // bubble 최소 크기
                .maxBubbleSize(40d); // bubble 최대 크기

        bubble.xAxis(0)
                .title("Average pulse during training")
                .minorTicks(true);// X축 보조눈금 설정
        bubble.yAxis(0)
                .title("Average power")
                .minorTicks(true); // Y축 보조눈금 설정
        ...
        anyBubbleChart.setChart(bubble);
    }
