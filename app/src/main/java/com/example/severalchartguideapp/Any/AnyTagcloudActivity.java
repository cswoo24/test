...
//https://github.com/AnyChart/AnyChart-Android/blob/master/sample/src/main/java/com/anychart/sample/charts/TagCloudActivity.java
public class AnyTagcloudActivity extends AnyChartActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_anytagcloud);

        AnyChartView anyChartView = findViewById(R.id.any_tag_cloud);

        //tag cloud(word cloud) : 단어의 빈도수에 비례해서 빈도수가 크면 글자크기는 크고 빈도수가 작으면 글자 크기도 작다
        TagCloud tagCloud = AnyChart.tagCloud();

        tagCloud.title("World Population");

        OrdinalColor ordinalColor = OrdinalColor.instantiate();
        ordinalColor.colors(new String[] { // 색 정의( 데이터 category 순서대로 지정. 처음 나온 아시아가 첫번째 색...)
                "#26959f", "#f18126", "#3b8ad8", "#60727b", "#e24b26"
        });
        tagCloud.colorScale(ordinalColor); //위에 정한 색으로 tag Cloud 색 지정
        tagCloud.angles(new Double[] {-90d, 0d, 90d}); // 단어들을 -90도, 0도, 90도 angle지정

        tagCloud.colorRange().enabled(true); // 색 범위 지정
        tagCloud.colorRange().colorLineSize(15d); // cloud하단 colorline 크기
