...
public class HelStackedActivity extends HelloChartActivity {

    private ColumnChartView stackedchart;
    private ColumnChartData data;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_helstacked);

        stackedchart = (ColumnChartView)findViewById(R.id.hel_stacked);
...
//	    data.setAxisXTop(axisX);  //x축 상단
        ...
        stackedchart.setInteractive(true); // 화면 움직이기 속성 설정
        stackedchart.setZoomType(ZoomType.HORIZONTAL);  //수평 이동
        stackedchart.setMaxZoom((float) 3);//화면 비율
        stackedchart.setColumnChartData(data); // chart에 라벨 설정까지 된 data insert
        stackedchart.setVisibility(View.VISIBLE); // 화면 보이기/안 보이기

       /* Viewport v = new Viewport(stackedchart.getMaximumViewport());
        v.left = 0;
        v.right= 7;
        stackedchart.setCurrentViewport(v);*/
        //stackedchart.setRotation(90);
    }
    ...
    private void generateData() {
        ...
        for (int i = 0; i < numColumns; ++i) {
            ...
            for (int j = 0; j < numSubcolumns; ++j) {
                values.add(new SubcolumnValue((float) Math.random() * 20f + 5, ChartUtils.COLORS[j]));
            }
