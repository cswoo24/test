...
public class HelNegSubColumnActivity extends HelloChartActivity {

    private ColumnChartView negsubcolumnchart;
    private ColumnChartData data;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_helnegsubcolumn);

        negsubcolumnchart = (ColumnChartView)findViewById(R.id.hel_neg_subcolumn);
...
//	    data.setAxisXTop(axisX);  //x축 상단
        ...
        negsubcolumnchart.setInteractive(true); // 화면 움직이기 속성 설정
        negsubcolumnchart.setZoomType(ZoomType.HORIZONTAL);  //수평 이동
        negsubcolumnchart.setMaxZoom((float) 3);//화면 비율
        negsubcolumnchart.setColumnChartData(data); // chart에 라벨 설정까지 된 data insert
        negsubcolumnchart.setVisibility(View.VISIBLE); // 화면 보이기/안 보이기
/*
        Viewport v = new Viewport(negsubcolumnchart.getMaximumViewport());
        v.left = 0;
        v.right= 7;
        negsubcolumnchart.setCurrentViewport(v);*/
    }
    ...
    private void generateData() {
        ...
        for (int i = 0; i < numColumns; ++i) {
            ...
            for (int j = 0; j < numSubcolumns; ++j) {
                int sign = getSign();
                values.add(new SubcolumnValue((float) Math.random() * 50f * sign + 5 * sign, ChartUtils.COLORS[j]
                        ));
            }
