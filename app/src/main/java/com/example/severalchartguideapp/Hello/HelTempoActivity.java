...
public class HelTempoActivity extends HelloChartActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout. activity_heltempo);

        chart = (LineChartView)findViewById(R.id.hel_tempo);
    ...
    private void generateTempoData() {
        ...
        line = new Line(values);
        line.setColor(ChartUtils.COLOR_RED); // line 색
        line.setHasPoints(false); // data value point 생성
        ...
        Axis tempoAxis = new Axis(axisValues).setName("Tempo [min/km]").setHasLines(true).setMaxLabelChars(4) // 왼쪽 Y축 이름,그리드 Y축 line
                .setTextColor(ChartUtils.COLOR_RED); // 색
        ...
        data.setAxisYRight(new Axis().setName("Height [m]").setMaxLabelChars(3) // 오른쪽Y축 이름, ?
                .setFormatter(new HeightValueFormatter(scale, sub, 0))); // 라벨 format
    ...
    private String formatMinutes(float value) { // time format & calculate
